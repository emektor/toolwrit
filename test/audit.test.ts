...
function buildChain(): AuditEntry[] {
  ...
  log.record(call('fs.read', { path: '/tmp/a' }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
  log.record(call('fs.write', { path: '/etc/x' }, T0 + 1), DENY, { calls: 1, tokens: 10, usd: 0.01 });
  log.record(call('fs.read', { path: '/tmp/b' }, T0 + 2), ALLOW, { calls: 1, tokens: 10, usd: 0.01 });
...
describe('canonicalize', () => {
  ...
  it('hashEntry inherits that stability, so key order cannot change a hash', () => {
    const common = { at: T0, run: 'r', decision: ALLOW, usage: { calls: 0, tokens: 0, usd: 0 }, prev: GENESIS };
...
describe('hashEntry', () => {
  it('is sha256 over the canonical form', () => {
    const body = { seq: 1, at: T0, run: 'r', tool: 't', args: {}, decision: ALLOW, usage: { calls: 0, tokens: 0, usd: 0 }, prev: GENESIS };
  ...
  it('changes when any field changes', () => {
    const body = { seq: 1, at: T0, run: 'r', tool: 't', args: { a: 1 }, decision: ALLOW, usage: { calls: 0, tokens: 0, usd: 0 }, prev: GENESIS };
    ...
    assert.notEqual(hashEntry({ ...body, usage: { calls: 1, tokens: 0, usd: 0 } }), base);
...
describe('AuditLog', () => {
  ...
  it('head() equals the last entry\'s hash, and GENESIS when empty', () => {
    ...
    const e1 = log.record(call('t', {}, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
    assert.equal(log.head(), e1.hash);
    const e2 = log.record(call('t', {}, T0 + 1), ALLOW, { calls: 1, tokens: 0, usd: 0 });
  ...
  it('a different run id produces a different chain', () => {
    ...
    const e = other.record(call('fs.read', { path: '/tmp/a' }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
    assert.notEqual(e.hash, buildChain()[0]!.hash);
  });

  it('history() exposes only non-denied calls, in the engine\'s shape', () => {
    const log = new AuditLog({ run: 'r' });
    log.record(call('fs.read', {}, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
    log.record(call('fs.write', {}, T0 + 1), DENY, { calls: 1, tokens: 0, usd: 0 });
    log.record(call('fs.read', {}, T0 + 2), { ...ALLOW, effect: 'ask' }, { calls: 1, tokens: 0, usd: 0 });
...
describe('verifyChain: the honest cases', () => {
  ...
  it('accepts a chain that is one legitimate entry longer', () => {
    const log = new AuditLog({ run: 'r' });
    for (let i = 0; i < 10; i++) {
      log.record(call('t', { i }, T0 + i), ALLOW, { calls: i, tokens: 0, usd: 0 });
...
describe('verifyFile', () => {
  it('verifies a JSONL log written by AuditLog', () => {
    ...
    log.record(call('fs.read', { path: '/tmp/a' }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
    log.record(call('fs.write', { path: '/etc/x' }, T0 + 1), DENY, { calls: 1, tokens: 0, usd: 0 });
  ...
  it('creates the parent directory for the log file', () => {
    ...
    log.record(call('t', {}, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
  ...
  it('detects an edit made directly to the file on disk', () => {
    ...
    const log = new AuditLog({ run: 'r', file });
    log.record(call('fs.read', { path: '/tmp/a' }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
    log.record(call('fs.write', { path: '/etc/shadow' }, T0 + 1), DENY, { calls: 1, tokens: 0, usd: 0 });
  ...
  it('two runs appended to one file each verify only as a whole-file chain', () => {
    ...
    new AuditLog({ run: 'a', file }).record(call('t', {}, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
    new AuditLog({ run: 'b', file }).record(call('t', {}, T0 + 1), ALLOW, { calls: 0, tokens: 0, usd: 0 });
    const r = verifyFile(file);
    assert.equal(r.ok, false);
    assert.equal(r.failure!.seq, 2);
    assert.equal(r.failure!.reason, 'bad-sequence');
...
describe('AuditLog: redaction', () => {
  ...
  it('does NOT mutate the caller\'s args object', () => {
    ...
    log.record(call('api.call', args, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
  ...
  it('a redaction path that does not exist is a harmless no-op', () => {
    ...
    const entry = log.record(call('t', { a: { b: 1 }, list: [] }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
  ...
  it('the chain still verifies over redacted entries', () => {
    const log = new AuditLog({ run: 'r', redact: ['token'] });
    log.record(call('a', { token: 's1' }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 });
  ...
  it('the hash commits to the redacted shape, so two different secrets hash alike', () => {
    ...
    const mk = (secret: string) => {
      const log = new AuditLog({ run: 'r', redact: ['token'] });
      return log.record(call('a', { token: secret }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 }).hash;
    };
  ...
  it('a secret cannot be un-redacted by re-hashing: the plaintext never verifies', () => {
    const log = new AuditLog({ run: 'r', redact: ['token'] });
    const entry = structuredClone(log.record(call('a', { token: 'secret' }, T0), ALLOW, { calls: 0, tokens: 0, usd: 0 }));
