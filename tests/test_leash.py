...
from leash.audit.chain import GENESIS
from leash.audit.verify import verify_chain, verify_file
from leash.budget.ledger import TokenPrice
from leash.leash import Leash, LeashDenied
from leash.types import ArgConstraint, BudgetLimits, Decision, Policy, RateLimit, ToolCall
...
def denied(fn: Callable[[], Any]) -> LeashDenied:
    """Run ``fn``, requiring it to raise LeashDenied, and return the exception."""
    with pytest.raises(LeashDenied) as info:
        fn()
    return info.value
...
class TestConstruction:
    def test_generates_a_run_id_when_none_is_given_and_stamps_it_on_entries(self) -> None:
        a = Leash(allow_fs(), now=frozen_clock())
        b = Leash(allow_fs(), now=frozen_clock())
    ...
    def test_uses_an_explicit_run_id(self) -> None:
        leash = Leash(allow_fs(), run="run-42", now=frozen_clock())
        assert leash.run == "run-42"
        leash.guard("fs.read", {"path": "/tmp/x"}, lambda: 1)
        assert leash.entries()[0].run == "run-42"

    def test_starts_with_empty_usage_no_entries_and_head_at_genesis(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        usage = leash.usage()
        assert (usage.calls, usage.tokens, usage.usd, usage.started_at) == (0, 0, 0, None)
        assert leash.entries() == []
        assert leash.head() == GENESIS


class TestCheck:
    def test_evaluates_without_recording_counting_or_executing(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        assert leash.check("fs.read", {"path": "/tmp/x"}).effect == "allow"
        assert leash.check("shell.exec").effect == "deny"
        assert leash.check("fs.write").effect == "ask", "check reports the raw effect"
        assert leash.entries() == []
        assert leash.usage().calls == 0

    def test_defaults_args_to_an_empty_object(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        assert [v.constraint for v in leash.check("fs.read").violations] == ["required"]
...
class TestGuardAllow:
    def test_executes_the_function_and_returns_its_value(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        ran = []
        result = leash.guard("fs.read", {"path": "/tmp/x"}, lambda: ran.append(1) or "contents")
    ...
    def test_guard_async_awaits_an_async_function(self) -> None:
        ...
        async def body() -> int:
            ...
            return await leash.guard_async("fs.read", {"path": "/tmp/x"}, tool)

        assert asyncio.run(body()) == 7

    def test_guard_async_also_accepts_a_plain_function(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        result = asyncio.run(leash.guard_async("fs.read", {"path": "/tmp/x"}, lambda: 7))
        assert result == 7

    def test_increments_calls_and_starts_the_clock_at_the_call_time(self) -> None:
        leash = Leash(allow_fs(), now=fixed_clock(at(0), at(5)))
        leash.guard("fs.read", {"path": "/tmp/a"}, lambda: 1)
        assert (leash.usage().calls, leash.usage().started_at) == (1, at(0))
        leash.guard("fs.read", {"path": "/tmp/b"}, lambda: 1)
        assert (leash.usage().calls, leash.usage().started_at) == (2, at(0))
    ...
    def test_records_the_decision_before_executing(self) -> None:
        ...
        with pytest.raises(RuntimeError, match="disk on fire"):
            leash.guard("fs.read", {"path": "/tmp/x"}, explode)

        assert len(leash.entries()) == 1
        assert leash.entries()[0].decision.effect == "allow"
        assert leash.usage().calls == 1, "authorised and counted before it failed"

    def test_records_the_entry_with_the_tool_args_time_and_decision(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock(at(3)))
        leash.guard("fs.read", {"path": "/tmp/x"}, lambda: 1)
        entry = leash.entries()[0]
...
class TestGuardDeny:
    def test_raises_leash_denied_carrying_the_tool_decision_and_audit_hash(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        ran = []
        err = denied(
            lambda: leash.guard("shell.exec", {"cmd": "rm -rf /"}, lambda: ran.append(1))
        )
        ...
        assert "leash: shell.exec denied — shell is off limits" in str(err)

        assert len(leash.entries()) == 1
        assert err.audit_hash == leash.entries()[0].hash, "the hash quotes the refusal"
        assert err.audit_hash == leash.head()

    def test_does_not_increment_the_call_counter_but_does_record_the_refusal(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        denied(lambda: leash.guard("shell.exec", {}, lambda: 1))
        usage = leash.usage()
        assert (usage.calls, usage.tokens, usage.usd, usage.started_at) == (0, 0, 0, None)
        assert len(leash.entries()) == 1
        assert leash.entries()[0].decision.effect == "deny"

    def test_denies_an_unknown_tool_by_default_and_reports_it(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        err = denied(lambda: leash.guard("mystery.tool", {}, lambda: 1))
    ...
    def test_denies_an_allow_rule_near_miss_and_surfaces_the_failing_argument(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        err = denied(lambda: leash.guard("fs.read", {"path": "/etc/passwd"}, lambda: 1))
        assert [
    ...
    def test_denied_calls_do_not_consume_a_rate_limit(self) -> None:
        ...
        leash = Leash(p, now=frozen_clock())
        denied(lambda: leash.guard("other", {}, lambda: 1))  # not attributed to `limited`
        leash.guard("t", {}, lambda: 1)
        denied(lambda: leash.guard("t", {}, lambda: 1))
        assert leash.usage().calls == 1
...
class TestGuardAsk:
    def test_denies_when_no_handler_is_configured_fail_closed(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        ran = []
        err = denied(lambda: leash.guard("fs.write", {"path": "/tmp/x"}, lambda: ran.append(1)))
        ...
        assert leash.usage().calls == 0
        assert leash.entries()[0].decision.effect == "deny", "the refusal is recorded"
    ...
    def test_allows_when_the_handler_returns_true_and_records_the_upgrade(self) -> None:
        ...
        leash = Leash(allow_fs(), now=frozen_clock(), on_ask=on_ask)
        assert leash.guard("fs.write", {"path": "/tmp/x"}, lambda: "written") == "written"
    ...
    def test_denies_when_the_handler_returns_false(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock(), on_ask=lambda c, d: False)
        err = denied(lambda: leash.guard("fs.write", {"path": "/tmp/x"}, lambda: 1))
        assert err.decision.effect == "deny"
    ...
    def test_guard_async_awaits_an_async_handler(self) -> None:
        ...
        leash = Leash(allow_fs(), now=frozen_clock(), on_ask=on_ask)
        asyncio.run(
            leash.guard_async("fs.write", {}, lambda: order.append("executed"))
        )
    ...
    def test_the_handler_receives_the_exact_call_it_is_asked_about(self) -> None:
        ...
        leash = Leash(allow_fs(), now=frozen_clock(at(9)), on_ask=on_ask)
        denied(lambda: leash.guard("fs.write", {"path": "/tmp/secret"}, lambda: 1))
    ...
    def test_cannot_upgrade_a_budget_denial(self) -> None:
        ...
        leash = Leash(p, now=frozen_clock(), on_ask=lambda c, d: True)
        leash.guard("fs.read", {"path": "/tmp/x"}, lambda: 1)
        err = denied(lambda: leash.guard("fs.write", {"path": "/tmp/x"}, lambda: 1))
...
class TestMeteringAndBudgets:
    def test_meter_adds_tokens_and_derives_spend_from_the_price_table(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock(at(2)))
        leash.meter(1000, 500, TokenPrice(input=0.003, output=0.015))
        usage = leash.usage()
    ...
    def test_a_previously_allowed_call_is_denied_once_the_call_budget_trips(self) -> None:
        ...
        leash.guard("fs.read", {"path": "/tmp/a"}, lambda: 1)
        leash.guard("fs.read", {"path": "/tmp/b"}, lambda: 1)
        err = denied(lambda: leash.guard("fs.read", {"path": "/tmp/c"}, lambda: 1))
        assert err.decision.violations[0].constraint == "calls"
        assert leash.usage().calls == 2, "the refused call is not counted"
    ...
    def test_the_seconds_budget_trips_on_the_injected_clock_alone(self) -> None:
        ...
        leash = Leash(p, now=fixed_clock(at(0), at(29), at(30)))
        leash.guard("fs.read", {"path": "/tmp/a"}, lambda: 1)
        leash.guard("fs.read", {"path": "/tmp/b"}, lambda: 1)
        err = denied(lambda: leash.guard("fs.read", {"path": "/tmp/c"}, lambda: 1))
    ...
    def test_rate_limits_are_enforced_across_guarded_calls(self) -> None:
        ...
        leash = Leash(p, now=fixed_clock(at(0), at(1), at(2), at(120)))
        leash.guard("t", {}, lambda: 1)
        leash.guard("t", {}, lambda: 1)
        err = denied(lambda: leash.guard("t", {}, lambda: 1))
        assert err.decision.violations[0].constraint == "limit"
        leash.guard("t", {}, lambda: 1)  # the window has slid past both entries
        assert leash.usage().calls == 3
...
class TestAuditChainOverAWholeRun:
    def test_verifies_across_a_mixed_run(self) -> None:
        ...
        leash.guard("fs.read", {"path": "/tmp/a"}, lambda: 1)              # allow
        denied(lambda: leash.guard("shell.exec", {}, lambda: 1))           # deny by rule
        leash.guard("fs.write", {"path": "/tmp/b"}, lambda: 1)             # ask -> approved
        denied(lambda: leash.guard("fs.read", {"path": "/etc/x"}, lambda: 1))  # near miss
        leash.guard("fs.read", {"path": "/tmp/c"}, lambda: 1)              # allow (3rd call)
        denied(lambda: leash.guard("fs.read", {"path": "/tmp/d"}, lambda: 1))  # budget

        entries = leash.entries()
        ...
        assert result.head == leash.head()

    def test_head_equals_the_last_entrys_hash_after_every_step(self) -> None:
        leash = Leash(allow_fs(), now=frozen_clock())
        assert leash.head() == GENESIS
        for i in range(4):
            leash.guard("fs.read", {"path": f"/tmp/{i}"}, lambda: i)
            assert leash.head() == leash.entries()[-1].hash, f"after {i + 1} calls"
        assert len(leash.entries()) == 4
    ...
    def test_writes_and_verifies_a_jsonl_audit_file_with_redaction(self, tmp_path: Path) -> None:
        ...
        leash.guard("api.call", args, lambda: 1)
        leash.guard("api.call", args, lambda: 2)

        result = verify_file(str(file))
        assert result.ok is True, result.failure
        assert result.count == 2
        assert result.head == leash.head()
        assert leash.entries()[0].args["token"] == "[redacted]"
        assert leash.entries()[0].args["path"] == "/tmp/x"
    ...
    def test_two_instances_with_the_same_inputs_produce_the_same_chain(self) -> None:
        def build() -> str:
            leash = Leash(allow_fs(), run="same", now=fixed_clock(at(0), at(1)))
            leash.guard("fs.read", {"path": "/tmp/a"}, lambda: 1)
            denied(lambda: leash.guard("shell.exec", {}, lambda: 1))
            return leash.head()
