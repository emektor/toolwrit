...
/** Run `fn`, requiring it to throw ToolwritDenied, and return it. */
async function denied(fn: () => Promise<unknown>): Promise<ToolwritDenied> {
  try {
    await fn();
  } catch (err) {
    if (!(err instanceof ToolwritDenied)) {
      assert.fail(`expected ToolwritDenied, got ${(err as Error)?.constructor?.name}: ${String(err)}`);
    }
    return err;
  }
  return assert.fail('expected ToolwritDenied to be thrown, but the call succeeded');
}

describe('Toolwrit: construction', () => {
  it('generates a run id when none is given, and stamps it on entries', async () => {
    const a = new Toolwrit({ policy: ALLOW_FS, now: frozenClock() });
    const b = new Toolwrit({ policy: ALLOW_FS, now: frozenClock() });
  ...
  it('uses an explicit run id', async () => {
    const l = new Toolwrit({ policy: ALLOW_FS, run: 'run-42', now: frozenClock() });
  ...
  it('starts with empty usage, no entries and head() at GENESIS', () => {
    const l = new Toolwrit({ policy: ALLOW_FS, now: frozenClock() });
...
describe('Toolwrit.guard: allow', () => {
  ...
  it('increments usage().calls and starts the clock at the call time', async () => {
    const l = new Toolwrit({ policy: ALLOW_FS, now: fixedClock(at(0), at(5)) });
  ...
  it('records the entry with the tool, args, time and decision', async () => {
    const l = new Toolwrit({ policy: ALLOW_FS, now: frozenClock(at(3)) });
    await l.guard('fs.read', { path: '/tmp/x' }, () => 1);
...
describe('Toolwrit.guard: deny', () => {
  it('throws ToolwritDenied carrying the tool, the decision and the audit hash', async () => {
    const l = new Toolwrit({ policy: ALLOW_FS, now: frozenClock() });
    let ran = false;
    const err = await denied(() => l.guard('shell.exec', { cmd: 'rm -rf /' }, () => { ran = true; }));

    assert.equal(ran, false, 'the guarded function must never run');
    assert.equal(err.name, 'ToolwritDenied');
    ...
    assert.match(err.message, /toolwrit: shell\.exec denied — shell is off limits/);
  ...
  it('denied calls do not consume a rate limit', async () => {
    ...
    const l = new Toolwrit({ policy: p, now: frozenClock() });
...
describe('Toolwrit.guard: ask', () => {
  ...
  it('denies when the handler returns false', async () => {
    const l = new Toolwrit({ policy: ALLOW_FS, now: frozenClock(), onAsk: () => false });
  ...
  it('is not consulted for allow or deny decisions', async () => {
    let asked = 0;
    const l = new Toolwrit({ policy: ALLOW_FS, now: frozenClock(), onAsk: () => { asked++; return true; } });
    await l.guard('fs.read', { path: '/tmp/x' }, () => 1);
    await denied(() => l.guard('shell.exec', {}, () => 1));
  ...
  it('cannot upgrade a budget denial — the budget is checked before the ask', async () => {
    ...
    const l = new Toolwrit({ policy: p, now: frozenClock(), onAsk: () => true });
    await l.guard('fs.read', { path: '/tmp/x' }, () => 1);
    const err = await denied(() => l.guard('fs.write', { path: '/tmp/x' }, () => 1));
...
describe('Toolwrit: metering and budgets', () => {
  it('meter() adds tokens and derives spend from the price table', () => {
    const l = new Toolwrit({ policy: ALLOW_FS, now: frozenClock(at(2)) });
  ...
  it('the seconds budget trips on the injected clock alone', async () => {
    ...
    const l = new Toolwrit({ policy: p, now: fixedClock(at(0), at(29), at(30)) });
    await l.guard('fs.read', { path: '/tmp/a' }, () => 1);
    await l.guard('fs.read', { path: '/tmp/b' }, () => 1);
    const err = await denied(() => l.guard('fs.read', { path: '/tmp/c' }, () => 1));
  ...
  it('rate limits are enforced across guarded calls using the injected clock', async () => {
    ...
    const l = new Toolwrit({ policy: p, now: fixedClock(at(0), at(1), at(2), at(120)) });
    await l.guard('t', {}, () => 1);
    await l.guard('t', {}, () => 1);
...
describe('Toolwrit: audit chain over a whole run', () => {
  it('verifies across a mixed run of allows, denies, asks and budget refusals', async () => {
    const p: Policy = { ...ALLOW_FS, budget: { calls: 3 } };
    const l = new Toolwrit({ policy: p, run: 'mixed', now: fixedClock(at(0), at(1), at(2), at(3), at(4), at(5)), onAsk: () => true });
  ...
  it('two Toolwrit instances with the same inputs produce the same chain', async () => {
    const build = async () => {
      const l = new Toolwrit({ policy: ALLOW_FS, run: 'same', now: fixedClock(at(0), at(1)) });
      await l.guard('fs.read', { path: '/tmp/a' }, () => 1);
      await denied(() => l.guard('shell.exec', {}, () => 1));
      return l.head();
    };
