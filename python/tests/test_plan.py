...
from toolwrit.audit.verify import verify_chain
from toolwrit.toolwrit import Toolwrit
from toolwrit.policy.load import parse_policy
from toolwrit.types import BudgetWarning, Policy
...
def collect(
...
) -> tuple[Toolwrit, list[BudgetWarning]]:
    ...
    return toolwrit, warnings


class TestTheApprovedEnvelopeIsRecordedFirst:
    def test_writes_the_plan_as_entry_one(self) -> None:
        toolwrit, _ = collect(make_policy("  purpose: nightly sync\n  approvedBy: ergin"))
        first = toolwrit.entries()[0]

        assert first.seq == 1
        assert first.tool == "toolwrit:plan"
    ...
    def test_records_the_defaults_it_will_actually_use(self) -> None:
        toolwrit, _ = collect(make_policy("  purpose: p"))
        assert toolwrit.entries()[0].args["warnAt"] == [0.8, 0.95]

    def test_records_approved_by_as_null_rather_than_omitting_it(self) -> None:
        toolwrit, _ = collect(make_policy("  purpose: p"))
        assert toolwrit.entries()[0].args["approvedBy"] is None

    def test_writes_no_plan_entry_when_the_policy_declares_no_plan(self) -> None:
        p = parse_policy('version: "1"\ndefault: allow\nrules: []\n')
        assert Toolwrit(p, now=lambda: T0).entries() == []


class TestThresholdReporting:
    def test_fires_each_threshold_exactly_once(self) -> None:
        toolwrit, warnings = collect(make_policy("  purpose: p\n  warnAt: [0.5, 0.9]"))
        for i in range(10):
            toolwrit.guard("t", {}, lambda: i)
    ...
    def test_reports_each_budget_dimension_independently(self) -> None:
        ...
        toolwrit, warnings = collect(p)

        toolwrit.meter(400, 200)  # 600/1000 tokens
        toolwrit.spend(1.5)       # 1.5/2 usd
    ...
    def test_measures_the_time_dimension_from_the_first_metered_event(self) -> None:
        ...
        toolwrit, warnings = collect(p, now=lambda: clock["now"])

        toolwrit.spend(0)  # starts the clock, 0s elapsed
        assert warnings == []
    ...
    def test_stays_silent_when_the_policy_has_a_budget_but_no_plan(self) -> None:
        p = parse_policy('version: "1"\ndefault: allow\nbudget:\n  calls: 10\nrules: []\n')
        warnings: list[BudgetWarning] = []
        toolwrit = Toolwrit(p, now=lambda: T0, on_warn=warnings.append)
    ...
    def test_puts_every_warning_in_the_chain_which_still_verifies(self) -> None:
        ...
        warned = [e for e in toolwrit.entries() if e.tool == "toolwrit:warning"]
        assert len(warned) == 2
