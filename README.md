...
The package installs a binary named `toolwrit`. The Python port speaks the same
policy language and writes a compatible audit chain — see
[`js/docs/python.md`](js/docs/python.md). Until it is on PyPI, install it from the
repository; it installs the same `toolwrit` binary:
...
Full reference with a worked example per constraint: [`js/docs/policy-reference.md`](js/docs/policy-reference.md).
...
See [`CONTRIBUTING.md`](js/CONTRIBUTING.md). The short version: no new runtime dependencies, and any change to enforcement semantics needs a test that fails without it.
